...
import { updateSampleRequest } from "@/app/admin/[storefront]/sample-requests/actions";
...
export default async function SampleRequestDetailPage({
  ...
  const errorMessage =
    resolvedSearchParams?.error === "shipping-required"
  ...
  if (error) {
    console.error("Unable to load sample request:", error);

    throw new Error(
      error.message || "Unable to load sample request."
    );
  }
  ...
  const addressLines = formatAddress(sampleRequest.shipping_address);
      ...
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-black/50">
            Requested {formatDate(sampleRequest.created_at)}
          </p>
        </div>
        ...
        <span
          className={[
            ...
            statusStyles[sampleRequest.status] || "bg-black/5 text-black"
          ].join(" ")}
        >
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
        <div className="grid gap-6">
          ...
          <section className="rounded-[2rem] bg-white p-5 shadow-soft md:p-6">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
              ...
              <div className="rounded-2xl bg-[#f8f6f1] p-4">
                ...
                <div className="mt-3 font-bold text-black">
                  {sampleRequest.business_name || "Not provided"}
                </div>

                <div className="mt-2 text-sm text-black/65">
                  {formatBusinessType(sampleRequest.business_type)}
                </div>
          ...
          <section className="rounded-[2rem] bg-white p-5 shadow-soft md:p-6">
            ...
            <div className="mt-5 rounded-2xl bg-[#f8f6f1] p-4 leading-7 text-black">
              {sampleRequest.notes || "No notes were submitted."}
            </div>
          </section>
          ...
          <section className="rounded-[2rem] bg-white p-5 shadow-soft md:p-6">
            ...
            <div className="mt-6 grid gap-4">
              <TimelineItem
                label="Submitted"
                value={sampleRequest.created_at}
              />

              <TimelineItem
                label="Approved"
                value={sampleRequest.approved_at}
              />

              <TimelineItem
                label="Rejected"
                value={sampleRequest.rejected_at}
              />

              <TimelineItem
                label="Shipped"
                value={sampleRequest.shipped_at}
              />

              <TimelineItem
                label="Completed"
                value={sampleRequest.completed_at}
              />
            </div>
          </section>
        </div>
        ...
        <aside className="h-fit rounded-[2rem] bg-white p-5 shadow-soft md:p-6 lg:sticky lg:top-28">
          ...
          <p className="mt-2 text-sm leading-6 text-black/55">
            Update approval status, shipping information and internal notes.
          </p>
          ...
          <form action={updateSampleRequest} className="mt-6 grid gap-4">
            <input
              type="hidden"
              name="requestId"
            ...
            <label className="grid gap-2">
              ...
              <select
                name="status"
                defaultValue={sampleRequest.status}
            ...
            <label className="grid gap-2">
              ...
              <select
                ...
                defaultValue={sampleRequest.shipping_carrier || ""}
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 text-black outline-none focus:border-black"
              >
            ...
            <label className="grid gap-2">
              ...
              <input
                name="trackingNumber"
                defaultValue={sampleRequest.tracking_number || ""}
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 text-black outline-none focus:border-black"
            ...
            <label className="grid gap-2">
              <span className="text-sm font-bold text-black">Tracking URL</span>

              <input
                type="url"
                name="trackingUrl"
                defaultValue={sampleRequest.tracking_url || ""}
                className="h-12 rounded-2xl border border-black/10 bg-white px-4 text-black outline-none focus:border-black"
                placeholder="https://..."
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <textarea
                name="internalNotes"
                defaultValue={sampleRequest.internal_notes || ""}
                rows={5}
          ...
          {sampleRequest.shipping_carrier || sampleRequest.tracking_number ? (
            <div className="mt-6 border-t border-black/10 pt-5">
              ...
              <div className="mt-3 text-sm font-bold uppercase text-black">
                {sampleRequest.shipping_carrier || "Carrier not assigned"}
              </div>

              <div className="mt-1 break-all text-sm text-black/60">
                {sampleRequest.tracking_number || "No tracking number"}
              </div>

              {sampleRequest.tracking_url ? (
                <a
                  href={sampleRequest.tracking_url}
                  target="_blank"
                  rel="noreferrer"
    ...
    <div className="flex items-start gap-3">
      <div
        className={[
          ...
          completed ? "bg-green-600" : "bg-black/10"
        ].join(" ")}
      />
      ...
      <div>
        ...
        <div className="mt-1 text-xs text-black/45">
          {completed ? formatDate(value) : "Not yet"}
        </div>
      </div>
    </div>
  );
