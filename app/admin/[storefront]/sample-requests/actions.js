...
export async function updateSampleRequest(formData) {
  await requireAdmin();
  ...
  const shippingCarrier = cleanText(
  ...
  if (!VALID_CARRIERS.has(shippingCarrier)) {
    throw new Error(
  ...
  if (
    ...
    redirect(
      `/admin/ecoware/sample-requests/${requestId}` +
  ...
  if (statusChanged) {
    const emailEligibleStatuses = new Set([
      "approved",
      "rejected",
      "shipped",
      "completed"
