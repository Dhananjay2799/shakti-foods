...
export default async function ReviewsPage({ params, searchParams }) {
  ...
  const { data: allReviews, error: countsError } = await supabase
    .from("product_reviews")
    .select("status")
    .eq("storefront", storefront.id);

  if (countsError) {
    console.error("Unable to load review counts:", countsError);
  }
  ...
  for (const review of allReviews || []) {
    if (Object.prototype.hasOwnProperty.call(counts, review.status)) {
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-xs font-bold uppercase tracking-[.2em] text-black/40">
            {storefront.name} Customer Trust
          </div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-black/55">
            Moderate {storefront.name} customer reviews, approve public
            feedback, reject inappropriate submissions and manage
      ...
      {error ? (
        ...
        <div className="mt-8 rounded-[2rem] bg-white p-10 text-center shadow-soft">
          ...
          <p className="mt-3 text-black/55">
            No reviews exist in this status yet.
          </p>
        </div>
      ) : (
        <>
                  ...
                  {rows.map((review) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-5 text-right align-top">
                        <Link
                          href={`${basePath}/reviews/${review.id}`}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span
                    className={[
                      ...
                      statusStyles[review.status] || "bg-black/5 text-black"
                    ].join(" ")}
                  >
                    {statusLabels[review.status] || review.status}
                  </span>
