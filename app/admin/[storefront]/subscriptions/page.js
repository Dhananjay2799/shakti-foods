...
export default async function AdminSubscriptionsPage({ params, searchParams }) {
  ...
  const pastDueCount = rows.filter((row) => row.status === "past_due").length;
  const pausedCount = rows.filter((row) => row.status === "paused").length;
  const canceledCount = rows.filter(
    (row) => row.status === "canceled" || row.status === "ended"
  ).length;
    ...
    <main className="min-h-screen bg-[#f8f6f1] px-4 py-10 md:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl leading-7 text-black/55">
              Manage recurring Stripe and PayPal Subscribe &amp; Save customers for {storefront.name}.
            </p>
          </div>
        ...
        <section className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <SummaryCard label="Past Due" value={pastDueCount} />
          <SummaryCard label="Paused" value={pausedCount} />
          <SummaryCard label="Canceled" value={canceledCount} />
        </section>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {rows.map((subscription) => {
                const item = subscription.subscription_items?.[0] || null;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-5 font-semibold">
                      {providerLabel(subscription.payment_provider)}
                    </td>
                    ...
                    <td className="px-5 py-5 text-right">
                      <Link
                        href={`${basePath}/subscriptions/${subscription.id}`}
            ...
            return (
              ...
              >
                ...
                <div className="mt-5 grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="text-black/40">Provider</div>
                  ...
                  <div>
                    <div className="text-black/40">Recurring</div>
                  ...
                  <div>
                    <div className="text-black/40">Frequency</div>
                  ...
                  <div>
                    <div className="text-black/40">Next billing</div>
