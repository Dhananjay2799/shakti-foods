...
function getWeightLabel(product) {
  if (product.category === "Rice" && product.badge) {
...
function getPackSize(product) {
  return product.packSize || product.badge || null;
}

async function ensureCategory({ categoryId, categoryName, sortOrder }) {
...
async function migrateProduct(product) {
  ...
  const productPayload = {
    ...
    sku: null,
    title: product.name,
    short_description: product.subtitle || null,
    description: product.shortDescription || product.subtitle || null,
    category: categoryId,
    subcategory: null,
    price_cents: priceToCents(product.unitPrice),
    compare_at_price_cents: null,
    cost_cents: null,
    currency: "USD",
    pack_size: getPackSize(product),
    unit_label: product.category === "Rice" ? "bag" : "pack",
    weight_label: getWeightLabel(product),
    status: "active",
    is_active: true,
    is_featured: product.id === "rice-20",
    taxable: true,
    requires_shipping: true,
    seo_title: product.seo?.title || product.name,
    seo_description: product.seo?.description || product.subtitle || null,
    deleted_at: null,
    archived_at: null
  };
  ...
  // Safe inventory check: do not overwrite stock if inventory already exists
  ...
  /*
   * Existing catalog images are served via public/images (lib/data.js fallbacks).
   * Images managed dynamically later via Admin will be inserted into product_images.
   */
...
async function run() {
  ...
  await ensureCategory({
    ...
    categoryName: "Eco-Friendly Tableware",
    sortOrder: 2
  });
  ...
  console.log("Static product migration completed.");
}
