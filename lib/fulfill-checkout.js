...
export async function fulfillCheckoutSession(
  ...
  if (
    ...
    throw new Error(
      `Unsupported storefront: ${storefront}`
    );
  }
...
export async function fulfillPayPalOrder(
  ...
  /*
   * Validate PayPal metadata BEFORE capture.
   */
  ...
  const databaseOrderId =
    String(existingOrder.id || "").trim();

  const databaseReservationId =
    String(existingOrder.reservation_id || "").trim();

  console.log(
    "PayPal purchase-unit validation:",
    {
      paypalOrderId:
        normalizedPayPalOrderId,
      paypalReferenceId,
  ...
  if (
    ...
    throw new Error(
      "PayPal purchase-unit reference ID does not match the internal order ID."
    );
  }
  ...
  /*
   * Capture only if PayPal hasn't already captured it.
   */
  if (
    paypalOrder?.status !==
    "COMPLETED"
  ...
  if (
    ...
    throw new Error(
      `PayPal order ${normalizedPayPalOrderId} is not completed.`
    );
  }
